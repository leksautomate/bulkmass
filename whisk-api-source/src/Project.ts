...
import type { ImageGenerationModelType, PromptConfig, MediaReference } from "./Types.js";
...
export class Project {
     ...
     * @param userCaption Optional custom caption to use instead of generating one
     */
    async addSubject(rawBytes: string, userCaption?: string) {
        await this.addReference(rawBytes, MediaCategory.SUBJECT, this.subjects, userCaption);
    }
     ...
     * @param userCaption Optional custom caption to use instead of generating one
     */
    async addScene(rawBytes: string, userCaption?: string) {
        await this.addReference(rawBytes, MediaCategory.SCENE, this.scenes, userCaption);
    }
     ...
     * @param userCaption Optional custom caption to use instead of generating one
     */
    async addStyle(rawBytes: string, userCaption?: string) {
        await this.addReference(rawBytes, MediaCategory.STYLE, this.styles, userCaption);
    }

    private async addReference(rawBytes: string, category: string, target: MediaReference[], userCaption?: string) {
        if (!(rawBytes?.trim?.())) {
            throw new Error("image data is required")
        }

        let caption = userCaption?.trim() || "";

        if (!caption) {
            const captions = await request<any>(
            ...
            caption = captions.candidates[0].output;
        }
    ...
    async generateImageWithReferences(input: string | PromptConfig): Promise<Media> {
        ...
        const generationResponse = await request<any>(
            ...
            body: JSON.stringify({
                ...
                "seed": input.seed,
                "imageModelSettings": {
                    "imageModel": "GEM_PIX", // required by image recipe
                    "aspectRatio": input.aspectRatio
                },
